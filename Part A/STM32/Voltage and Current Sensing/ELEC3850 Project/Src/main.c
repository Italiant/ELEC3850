...
static volatile uint32_t g_ADCBuffer[2];
uint32_t g_ADC1, g_ADC2;

//Variables for Moving Average Filter
enum{ Window_Size = 10 };
float buf[Window_Size];					//Buffer for MotorCurrent Moving Average Filter
int n = 0;								//Counter for buffer
int m = 0;								//Counter for moving average filter iterations
float MC = 0;							//Motor current
float sum = 0;							//Rolling sum for MC filter

char usbCdcTxBuffer[2000];

void vcp_transmit(uint8_t *s)
{
...
float Motor_Current(uint32_t g_ADC2){
	...
	//When buffer is full, filter and output result
		...
		sum = sum + buf[n-1];
		MC = (((sum/Window_Size)*3.3/4095.0)-2.5)*(1/0.025);		//Calculate average
	...
	//Filter all results from now on (Moving Average)
		...
		MC = (((sum/Window_Size)*3.3/4095.0)-2.5)*(1/0.025);	//Motor Current (A) = 2.5 +- 25mV/A;
		if(n < (Window_Size)){
			sum = sum - buf[n];
		}
		else{						//End of buffer
